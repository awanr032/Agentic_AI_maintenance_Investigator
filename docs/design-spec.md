...
- **AWS — actually done (Extraction and Query Agents):** `infra/terraform/` provisions two real Lambda deployments, each with its own separately-scoped IAM role (least privilege: neither role has a permission the other agent doesn't specifically need).
  ...
  - **Query Agent**: role additionally has S3 read/write on a dedicated store bucket (`infra/terraform/store.tf`), populated via `scripts/migrate_store_to_s3.py`. This is what made `store.py`'s S3 backend (§6, below) real rather than theoretical — the whole point of building it. **A real deployment bug was found and fixed on the first live test**: `tools.py`'s `_validated_pass_records()` scans the *entire* split on every tool call (the actual read pattern Pattern/Query Agent use, not a rare case) — locally that's one fast file read, but the S3 backend's one-object-per-record layout turned it into ~500 sequential network round-trips for the silver split, timing out the Lambda at its original 30s limit. Fixed by parallelizing the reads in `iter_records()`'s S3 branch (verified: 500 records in ~4s from outside AWS, faster still from within it) and raising the timeout to 60s as a margin on top of the real fix, not instead of it.
  - Both verified end-to-end via direct `aws lambda invoke`, matching local output exactly.
  - **Known gap, not yet resolved, shared by both**: each Lambda's public Function URL returns 403 despite a provably correct IAM resource policy — most likely a new-AWS-account anti-abuse restriction on anonymous Function URL access (the account also showed a below-default concurrency limit of 10). Worked around by demoing via direct invocation (CLI/console) instead of a public URL; not blocking, since direct invocation is a normal, legitimate way to call a Lambda.
  - **Not yet done**: Validation and Drafting Agents are still local-only. Pattern Agent is local-only too but would need no new infrastructure to deploy — it would reuse the exact S3-backed `store.py` and IAM pattern Query Agent already proves out.
