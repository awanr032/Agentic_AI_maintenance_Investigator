...
- Only use types from the allowed lists above. If nothing in the text fits an allowed type, do not force a label — omit that entity or relation.
...
- When a multi-word noun phrase names one physical object at increasing specificity — a modifier plus a head noun, where the head noun alone is also a valid, more general name for the same kind of thing (e.g. "bend pulley", where "pulley" is the general head noun and "bend" narrows it) — extract BOTH the full phrase AND the bare head noun as separate entities, and connect them with an "isA" relation (full phrase isA head noun). Do not extract only the full phrase as one atomic entity. Example: for the phrase "bend pulley", extract {{"span_text": "bend pulley", "type": "PhysicalObject/GuidingObject/MechanicalEnergyGuidingObject", "confidence": "high"}} AND {{"span_text": "pulley", "type": "PhysicalObject/GuidingObject/MechanicalEnergyGuidingObject", "confidence": "high"}}, plus {{"head_span": "bend pulley", "tail_span": "pulley", "type": "isA"}}.
- Similarly, when a multi-word noun phrase names an assembly made of a named larger unit plus a named sub-part (e.g. "PTO shaft" = a "PTO" that has a "shaft" as part of it), extract each named unit as its own entity and connect them with "hasPart" (larger unit hasPart sub-part), rather than treating the whole phrase as one atomic entity.
