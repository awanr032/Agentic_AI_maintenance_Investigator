...
- Even if a word is unfamiliar heavy-equipment jargon you don't specifically recognize (e.g. an uncommon part name), still pick your single most plausible SPECIFIC subtype based on general context — is it likely a mechanical part, a fastener, a fluid-handling component? — and mark it "low" confidence. Only use the bare top-level category when the word clearly IS a PhysicalObject/Activity/etc. but you cannot imagine any specific subtype fitting even loosely — an unfamiliar name is a reason to guess low-confidence, not a reason to give up on specificity.
- Filters, ripper tips/teeth, pre-cleaners, and dryers (anything that filters, separates, crushes, or removes something from a substance) are typically PhysicalObject/MatterProcessingObject subtypes (e.g. MechanicalSeparatingObject for filters/pre-cleaners, GrindingAndCrushingObject for ripper tips and similar cutting/crushing tools, ThermalSeparatingObject for dryers) — not a bare PhysicalObject.
- In heavy-equipment maintenance text, "swing" (as in "swing motor", "swingbox", "swing pedestal") refers to a rotation/slewing mechanism — PhysicalObject/TransformingObject/MechanicalEnergyTransformingObject — not a bare PhysicalObject.
- "Oil", "coolant", or "lube"/"lubricant" in a maintenance context is classified by its PROTECTIVE FUNCTION, not as a bare Substance/Liquid: lubricating oil/lube protects against wear/friction (PhysicalObject/ProtectingObject/WearProtectingObject), coolant protects against overheating (PhysicalObject/ProtectingObject/TemperatureProtectingObject). Only use PhysicalObject/Substance/Liquid for a fluid that ISN'T playing a protective role (e.g. "fuel" being burned/consumed).
